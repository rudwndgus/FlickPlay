...
export class NeonVaultController implements GameController {
  ...
  private enemy = { x: 4, y: 11, direction: 1 }; private dying = 0; private clearing = 0; private shake = 0; private impactPulse = 0
  ...
  reset() {
    ...
    this.path = []; this.pathIndex = 0; this.segmentClock = 0; this.drag = null; this.autoClock = 0; this.enemy = { x: 4, y: 11, direction: 1 }
    this.dying = 0; this.clearing = 0; this.shake = 0; this.impactPulse = 0; this.moveDirection = { x: 0, y: -1 }; this.toast = 'COLLECT SIGNALS • HIT 2 SWITCHES'; this.toastLife = 2.4; this.collectedCoins = 0; this.trail = []; this.particles = []; this.coins = new Set(COINS); this.switches = new Set(SWITCHES); this.dots = new Set()
    const reserved = new Set([`${START.x},${START.y}`, `${EXIT.x},${EXIT.y}`, ...COINS, ...PORTALS.keys(), ...SPIKES.map((s) => `${s.x},${s.y}`), ...SWITCHES, '4,11', '3,9'])
  ...
  private laserCell(x: number, y: number) { return y === 11 && x >= 5 && x <= 10 }
  ...
  private visit(node: Node) {
    ...
    if (this.switches.delete(key)) { this.addScore(5); this.burst(node.x, node.y, '#19f7e7', 16); this.shake = .16; this.toast = this.switches.size ? 'SWITCH 1/2 • FIND THE OTHER' : 'ALL SWITCHES ACTIVE'; this.toastLife = 1.6; this.options.onImpact('perfect') }
    ...
    if (node.x === EXIT.x && node.y === EXIT.y) { if (this.dots.size || this.switches.size) { this.toast = this.switches.size ? `${this.switches.size} SWITCHES • ${this.dots.size} SIGNALS` : `${this.dots.size} SIGNALS LEFT`; this.toastLife = 1.5 } else { this.player.moving = false; this.clearing = .72; this.burst(node.x, node.y, '#54ffe0', 24) } }
  }
  ...
  update(dt: number) {
    ...
    this.move(dt); this.enemy.x += this.enemy.direction * 1.35 * dt; if (this.enemy.x > 10.6 || this.enemy.x < 3.4) { this.enemy.x = clamp(this.enemy.x, 3.4, 10.6); this.enemy.direction *= -1 }
    if (!this.dying && Math.hypot(this.player.x - this.enemy.x, this.player.y - 11) < .36) this.danger(this.player.x, this.player.y)
  ...
  render(ctx: CanvasRenderingContext2D) {
    ...
    for (const key of SWITCHES) { const [c, r] = key.split(',').map(Number), p = this.point(c, r), active = !this.switches.has(key); ctx.save(); ctx.translate(p.x, p.y); ctx.rotate(Math.PI / 4); ctx.fillStyle = active ? '#19f7e7' : '#ffb21a'; ctx.shadowColor = ctx.fillStyle; ctx.shadowBlur = active ? 18 : 10; ctx.fillRect(-6, -6, 12, 12); ctx.fillStyle = '#030308'; ctx.fillRect(-2, -2, 4, 4); ctx.restore() }
    ...
    const ls = this.point(5, 11), le = this.point(10, 11); ctx.strokeStyle = this.laserActive() ? '#00f5df' : 'rgba(0,245,223,.28)'; ctx.lineWidth = this.laserActive() ? 5 : 2; if (!this.laserActive()) ctx.setLineDash([5, 6]); ctx.beginPath(); ctx.moveTo(ls.x - 9, ls.y); ctx.lineTo(le.x + 9, le.y); ctx.stroke(); ctx.setLineDash([])
    ...
    const exit = this.point(EXIT.x, EXIT.y), open = !this.dots.size && !this.switches.size; ctx.strokeStyle = open ? '#54ffe0' : '#a52b55'; ctx.lineWidth = 4; ctx.beginPath(); ctx.arc(exit.x, exit.y, 10, this.elapsed * 2, this.elapsed * 2 + 5); ctx.stroke(); if (!open) { ctx.fillStyle = '#ff668a'; ctx.font = '800 7px system-ui'; ctx.textAlign = 'center'; ctx.fillText(String(this.dots.size + this.switches.size), exit.x, exit.y + 3) }
    const enemy = this.point(this.enemy.x, 11); ctx.save(); ctx.translate(enemy.x, enemy.y); ctx.rotate(this.elapsed + .8); ctx.fillStyle = '#16f5e6'; ctx.shadowColor = '#00f5e1'; ctx.shadowBlur = 20; ctx.fillRect(-7, -7, 14, 14); ctx.fillStyle = '#032b2a'; ctx.fillRect(-3, -3, 6, 6); ctx.restore()
