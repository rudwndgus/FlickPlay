...
const definitions: Definition[] = [
  ...
  },
  {
    ...
    shortDescription: '도끼 하나를 새총처럼 날려 암벽에 박고, 박힌 위치에서 다시 도약해 끝없는 첨탑을 정복하세요.',
    fullDescription: '사람이나 연결줄 없이 도끼 자체로 플레이합니다. 화면 어디서든 새총처럼 뒤로 당기면 도끼가 반대 방향으로 날아가며, 날이 바위나 나무에 올바른 각도와 속도로 닿으면 그 자리가 다음 투척 지점이 됩니다. 금속·수정·얼음·가시 바위에는 박히지 않고 실제로 튕겨 나갑니다.',
    ...
    controls: [{ label: '화면 어디서든 당기기', description: '도끼를 직접 잡을 필요 없이 화면 아무 곳에서나 원하는 발사 방향의 반대로 당깁니다.' }, { label: '놓기', description: '아래로 당기면 위로, 오른쪽으로 당기면 왼쪽으로 도끼가 날아갑니다. 당긴 거리가 힘을 결정하며 18px보다 짧으면 취소됩니다.' }, { label: '공중 재조준', description: '도끼가 튕기거나 추락하는 중에도 다시 당겨 새로운 방향으로 날릴 수 있습니다.' }],
    scoringRules: [{ label: '최고 높이', value: '시작 바닥 0m부터 정상 1000m까지 도끼가 도달한 최고 높이를 기록' }, { label: '정상 도달', value: '도끼가 정상의 빛나는 제단에 닿거나 박히면 1000m 완주' }, { label: '추락', value: '점수는 유지되지만 도끼가 실제 아래 지형에 걸릴 때까지 올라온 거리를 잃음' }],
    failureConditions: ['게임 오버와 즉사는 없습니다. 잘못 던지면 아래 지형 또는 시작 바닥까지 실제로 추락합니다.'],
    tips: ['바위와 나무의 정면을 향해 충분한 속도로 날을 꽂아야 하며 손잡이 충돌이나 스치는 각도는 튕겨 나갑니다.', '도끼가 박힌 위치가 곧 다음 발사점이므로 다음 암벽을 볼 수 있는 면을 노리세요.', '금속은 청색, 수정은 민트색, 얼음은 옅은 하늘색으로 빛나며 모두 도끼가 박히지 않습니다.', '큰 추락 중에는 넓은 측면 암벽을 향해 빠르게 다시 발사해 복구하세요.'],
  },
]
