...
FlickPlay!는 Instagram Reels처럼 위아래로 넘기며 게임을 발견하고, 미리보기 화면을 탭해 즉시 플레이하는 모바일 우선 PWA입니다. 별도의 스플래시 화면 없이 첫 게임 피드를 바로 보여주며, 피드 위치와 최고 기록·좋아요·즐겨찾기를 기기에 보존합니다.
...
`vite-plugin-pwa`의 `prompt` 업데이트 방식을 사용합니다. 새 서비스 워커가 준비되면 사용자가 플레이를 마친 뒤 업데이트할 수 있으며 IndexedDB의 최고 점수와 개인 설정은 유지됩니다. iPhone에서는 Safari 공유 메뉴의 **홈 화면에 추가**를 사용합니다.
